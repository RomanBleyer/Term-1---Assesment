...
from background_functions import quiz_database_initialisation, clear_screen, fancy_clear_screen, title_typewriter, border_effect, title_screen_typewriter_text, title_screen_glitched, title_screen_very_glitched, title_screen_very_very_glitched
...
def quiz_type_select(selected_categories, quiz_database, total_questions_attempted, total_questions_correct):
    ...
    while True:
        try:
            ...
            if user_choice_two.isdigit():
                user_choice_two = int(user_choice_two)
                if 1 <= user_choice_two <= len(available_categories):
                    selected_category = available_categories[user_choice_two - 1]
